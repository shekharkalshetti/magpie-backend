...
echo ""
log_info "🔒 Red Teaming Feature:"
echo "   • 20 pre-built attack templates available"
echo "   • Templates located in: src/red_teaming/templates/"
echo "   • Queue: red_team (processed by Celery worker)"
echo "   • Test via: http://localhost/docs (Red Teaming endpoints)"
echo ""
log_info "📊 View Logs:"
...
echo "   Check queues:      docker compose exec worker celery -A src.tasks.celery_app inspect active_queues"
echo ""
log_info "🛑 Teardown:"
...
if [ "$HAS_GPU" = false ]; then
    log_warning "⚠️  LLM service not running (no GPU)"
